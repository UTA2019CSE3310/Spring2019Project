...
void page_usr_enter(UI* ui, int row, int col, char* buf);
...
void make_login(UI* ui, int row, int col)
{
	char* line1 = "Welcome to SuperChat!";
	char* line2 = "Please enter a nick to begin:";
    ...
    mvwaddch(ui->title, wy, wx, ACS_ULCORNER); 
	mvwhline(ui->title, wy, wx+1, ACS_HLINE, 32);
	mvwaddch(ui->title, wy,wx + 33, ACS_URCORNER);
	
	mvwvline(ui->title, wy+1, wx, ACS_VLINE, 8); 
	mvwvline(ui->title, wy+1, wx+33, ACS_VLINE, 8); 
	
	mvwaddch(ui->title, wy+9, wx, ACS_LLCORNER); 
	mvwhline(ui->title, wy+9, wx+1, ACS_HLINE, 32);
	mvwaddch(ui->title, wy+9,wx + 33, ACS_LRCORNER);
    
    
    wattron(ui->title, COLOR_PAIR(3));
	wx = centered(line1, wx, 33);
	mvwprintw(ui->title, wy+2, wx, "%s", line1);

	wx = centered(line2, wx, 30);
	mvwprintw(ui->title, wy+2, wx-3,"%s", line2);
	wattroff(ui->title, COLOR_PAIR(3));
	
	/* Paint the input window */
	ui->input = newwin(1, 26, wy+7, wx - 2);
	keypad(ui->input, TRUE);
	wmove(ui->input, 0, 0);
	
	mvwaddch(ui->title, wy+6, wx-3, ACS_ULCORNER); 
	mvwhline(ui->title, wy+6, wx-2, ACS_HLINE, 26);
	mvwaddch(ui->title, wy +6,wx + 24, ACS_URCORNER);
	
	mvwaddch(ui->title, wy +7,wx -3, ACS_VLINE);
	mvwaddch(ui->title, wy +7,wx + 24, ACS_VLINE);
	
	mvwaddch(ui->title, wy+8, wx-3, ACS_LLCORNER); 
	mvwhline(ui->title, wy+8, wx-2, ACS_HLINE, 26);
	mvwaddch(ui->title, wy+8,wx + 24, ACS_LRCORNER);
